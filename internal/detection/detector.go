...
// Detector runs YOLOv8 person detection using ONNX Runtime.
...
func NewDetector(modelPath string, threshold float32, modelVer string, logger *slog.Logger) (*Detector, error) {
	...
	// Define input/output shapes for YOLOv8n
	...
	// YOLOv8 output: [1, 84, 8400] — 84 = 4 bbox + 80 classes, 8400 detections
...
func (d *Detector) Detect(imagePath string, rootID, relPath string, mtime int64, size int64) (*Result, error) {
	...
	hasPerson, confidence := d.parseYOLOv8Output(outputData)
...
// parseYOLOv8Output parses YOLOv8 output [1, 84, 8400] for person detections.
// The output is transposed compared to YOLOv5: each of 8400 detections has 84 values
// (4 bbox coords + 80 class scores). Class 0 = person.
func (d *Detector) parseYOLOv8Output(output []float32) (hasPerson bool, maxConf float32) {
	const numDetections = 8400
	const personIdx = 4 + cocoPersonClass // bbox(4) + person class(0)

	for i := 0; i < numDetections; i++ {
		// YOLOv8 output is [1, 84, 8400], stored as 84 rows of 8400
