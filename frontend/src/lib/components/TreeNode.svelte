...
interface Props {
	...
	// Toast notifications bubble up to TreeView, which mounts the single
	// Toast container for the whole tree (one per expanded TreeNode would
	// render a stack of overlapping fixed-position containers).
...
async function handleDelete() {
	...
	try {
		...
		onToast?.('Deleted 1 item(s)', 'success');
...
function handleCut() {
	...
	onToast?.('Cut 1 item(s)', 'success');
}

function handleCopy() {
	if (!clipboard) return;
	clipboard.copy(rootId, [entry.path]);
	onToast?.('Copied 1 item(s)', 'success');
}
...
async function handlePaste() {
	...
	try {
		...
		if (errors.length > 0) {
			onToast?.(`${errors.length} item(s) failed to ${mode}`, 'error');
		} else {
			onToast?.(
		...
		onToast?.(e instanceof Error ? e.message : 'Paste failed', 'error');
	}
}
